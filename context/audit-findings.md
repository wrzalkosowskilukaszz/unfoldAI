...
| # | Finding | Status |
|---|---|---|
| 1 | **Rate limiting does not work on serverless.** `rateLimit.ts` keeps buckets in an in-process `Map`; Vercel runs many instances, each with its own memory. 25/10min is per-instance, not per person. The Anthropic spend cap is the only real backstop. Fix: shared store (Vercel KV / Upstash), keep in-memory as the local-dev fallback. | ☐ open |
